...
namespace BlogApp.Services.Concrete
{
    public class ArticleManager : ManagerBase,IArticleService
    {
        ...
        public async Task<IDataResult<ArticleDto>> GetAsync(int articleId)
        {
            ...
            return new DataResult<ArticleDto>(ResultStatus.Error, ArticleMessages.NotFoundSingular, null);
        }
        ...
        {
            List<Expression<Func<Article, bool>>> predicactes = new List<Expression<Func<Article, bool>>>();
            ...
            predicactes.Add(a=>a.Id==articleId);
            var article = await UnitOfWork.Articles.GetAsyncV2(predicactes,includes);
            if (article == null)
            {
                return new DataResult<ArticleDto>(ResultStatus.Warning,
                    GeneralMessages.ValidationError(),null,new List<ValidationError>
                    {
                        new ValidationError
                        {
                            PropertyName = "articleId",
                            Message = ArticleMessages.NotFoundById(articleId)
                        }
                    });
            }
            return new DataResult<ArticleDto>(ResultStatus.Success, new ArticleDto
        ...
        public async Task<IDataResult<ArticleUpdateDto>> GetArticleUpdateDtoAsync(int articleId)
        {
            ...
            {
                return new DataResult<ArticleUpdateDto>(ResultStatus.Error, ArticleMessages.NotFoundSingular, null);
            }
        }
            ...
            OrderByGeneral orderBy, bool isAscending, bool includeCategory, bool includeComments, bool includeUser)
        {
            ...
            if (categoryId.HasValue)
            {
                ...
                {
                    return new DataResult<ArticleListDto>(ResultStatus.Warning,
                        GeneralMessages.ValidationError(), null, new List<ValidationError>
                        {
                            new ValidationError
                            {
                                PropertyName = "categoryId",
                                Message = CategoryMessages.NotFoundById(categoryId.Value)
                            }
                        });
                }
                predicactes.Add(a=>a.CategoryId==categoryId.Value);
            }
            if (userId.HasValue)
            {
                if (!await _userManager.Users.AnyAsync(u=>u.Id==userId.Value))
                {
                    return new DataResult<ArticleListDto>(ResultStatus.Warning,
                        GeneralMessages.ValidationError(), null, new List<ValidationError>
                        {
                            ...
                            {
                                PropertyName = "userId",
                                Message = UserMessages.NotFoundById(userId.Value)
                            }
                        });
                }
                predicactes.Add(a => a.UserId == userId.Value);
            }
            if(isActive.HasValue) predicactes.Add(a=>a.IsActive==isActive.Value);
            if(isDeleted.HasValue) predicactes.Add(a=>a.IsDeleted==isDeleted.Value);
            //includes
            if (includeCategory) includes.Add(a => a.Category);
            if (includeComments) includes.Add(a => a.Comments);
            if (includeUser) includes.Add(a => a.User);

            var articles = await UnitOfWork.Articles.GetAllAsyncV2(predicactes, includes);
        ...
        public async Task<IDataResult<ArticleListDto>> GetAllAsync()
        {
            ...
            return new DataResult<ArticleListDto>(ResultStatus.Error, ArticleMessages.NotFoundPlural, null);
        }
        ...
        public async Task<IDataResult<ArticleListDto>> GetAllByCategoryAsync(int categoryId)
        {
            ...
            return new DataResult<ArticleListDto>(ResultStatus.Error, CategoryMessages.CategoryNotFoundSingular, null);
        ...
        public async Task<IResult> IncreaseViewCountAsync(int articleId)
        {
            ...
            if (article == null)
            {
                return new Result(ResultStatus.Error, ArticleMessages.NotFoundSingular);
            }

            article.ViewCount += 1;
            await UnitOfWork.Articles.UpdateAsync(article);
            await UnitOfWork.SaveAsync();
            return new Result(ResultStatus.Success, ArticleMessages.IncreaseViewCount(article.Title));
        }
        ...
        public async Task<IResult> AddAsync(ArticleAddDto articleAddDto, string createdByName,int userId)
        {
            ...
            await UnitOfWork.SaveAsync();
            return new Result(ResultStatus.Success, ArticleMessages.Add(article.Title));
        }
        ...
        public async Task<IResult> UpdateAsync(ArticleUpdateDto articleUpdateDto, string modifiedByName)
        {
            ...
            await UnitOfWork.Articles.UpdateAsync(article);
            await UnitOfWork.SaveAsync();
            return new Result(ResultStatus.Success, ArticleMessages.Update(article.Title));
        }
        ...
        public async Task<IResult> DeleteAsync(int articleId, string modifiedByName)
        {
            ...
            if (result)
            {
                ...
                await UnitOfWork.Articles.UpdateAsync(article);
                await UnitOfWork.SaveAsync();
                return new Result(ResultStatus.Success, ArticleMessages.Delete(article.Title));
            }
            return new Result(ResultStatus.Error, ArticleMessages.NotFoundSingular);
        }
        ...
        public async Task<IResult> HardDeleteAsync(int articleId)
        {
            ...
            if (result)
            {
                ...
                await UnitOfWork.SaveAsync();
                return new Result(ResultStatus.Success, ArticleMessages.HardDelete(article.Title));
            }
            return new Result(ResultStatus.Error, ArticleMessages.NotFoundSingular);
        }
        ...
        public async Task<IResult> UndoDeleteAsync(int articleId, string modifiedByName)
        {
            ...
            if (result)
            {
                ...
                await UnitOfWork.Articles.UpdateAsync(article);
                await UnitOfWork.SaveAsync();
                return new Result(ResultStatus.Success, ArticleMessages.UndoDelete(article.Title));
            }
            return new Result(ResultStatus.Error, ArticleMessages.NotFoundSingular);
        }
